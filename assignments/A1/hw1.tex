...
\midrule
0. Public-key registration (\textbf{due Sep 11}) & required (gate) & Gradescope (separate) \\
...
This part takes five minutes and has its own, \textbf{earlier} deadline. Run the provided tool (pure Python, no dependencies --- your agent can do this):
\begin{verbatim}
    python3 tools/anon-report.py keygen
\end{verbatim}
It creates a private key in \file{\textasciitilde/.anon-report/key.json} and writes \file{pubkey.txt} in the current directory. Submit \file{pubkey.txt} to the separate Gradescope assignment \textbf{``A1 Part 0: key registration''} by \textbf{Friday, September 11, 11:59 pm}.

\medskip\noindent\emph{Why:} these keys are how the anonymous agent-report channel works (\S7). On \textbf{Monday, September 14} we publish the roster of all registered public keys; your agent will then sign your report with a \emph{linkable ring signature} over that roster --- proving ``some enrolled student wrote this'' without revealing which --- and after the report deadline you will prove you submitted without revealing which report is yours. Later in the course you will build this scheme yourself. For now, just generate and register the key.
