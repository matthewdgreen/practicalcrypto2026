...
\noindent
\textbf{Released:} Wednesday, September 2, 2026 \hfill \textbf{Part 0 due:} Friday, September 11 \hfill
...
\medskip\noindent First, \textbf{by Friday September 11}, you will register a public key (Part 0). Then, for the assignment proper, you will:
...
\item \textbf{Python.} Write your submission in Python 3. If you want to use another language, ask the instructor for permission on Piazza \emph{before} you start: the course container has toolchains for several (see \S8), but course staff must be able to review your code. No Excel macros.
\item \textbf{Minimum viable submission:} you must pass the autograder's build-and-basic-correctness check to sign up for Review Lab 1. Review Lab 1 is graded separately (\S6) and does not change your score on this assignment.
\item Late hours per the syllabus apply to the Gradescope deadline, not to Review Lab sign-ups and not to Part 0.
...
\midrule
0. Public-key registration (\textbf{due Sep 11}) & required & Gradescope (separate) \\
...
\section*{Part 0 --- Register your public key (required; due Fri Sep 11, 11:59 pm)}

This part takes five minutes and has its own, \textbf{earlier} deadline. Run the provided tool on your own machine (pure Python, no dependencies. If you're using coding agents, your agent can do this):
\begin{verbatim}
    python3 tools/anon-report.py keygen
\end{verbatim}
It creates a private key in \file{\textasciitilde/.anon-report/key.json} and writes \file{pubkey.txt} in the current directory. \underline{Please do not lose your private key!} Submit \file{pubkey.txt} to the separate Gradescope assignment \textbf{``A1 Part 0: key registration''} by \textbf{Friday, September 11, 11:59 pm}.

\medskip\noindent\emph{Why:} you'll use this key to submit your anonymous agent report (see \S7). %On \textbf{Monday, September 14} we publish the roster of all registered public keys; your agent will then sign your report with a \emph{linkable ring signature} over that roster --- proving ``some enrolled student wrote this'' without revealing which --- and after the report deadline you will prove you submitted without revealing which report is yours. Later in the course you will build this scheme yourself. For now, just generate and register the key.

\medskip\noindent\emph{Rules:} keep the private key! You need it for every assignment this semester; back it up. \textbf{Never paste it into any note, report, chat, or submission.} If you lose it, you will re-register on the next roster. Part 0 carries no points but is \textbf{required for review-lab sign-up}.
...
\end{verbatim}
\emph{This part is glue. You should understand how the cipher works, so I recommend implementing it yourself. But nobody will ask you about it in the review.}
...
\item More than 10 candidates is penalized. 
...
Review Lab 1 is a 15-minute individual session with course staff during the week of September 21. You don't have to bring anything: your submission will be on the screen. This review is graded on a comprehension rubric. It counts toward the Review Lab component of your course grade (25\% overall, across three labs) and does not change your score on this assignment. You won't be asked about glue code.
...
\item \textbf{Modification ($\approx$5 min).} Make a small change live, or find a flaw that we have planted in a \emph{copy} of your own code. A planted flaw is always inside one of the three review-critical regions above, and we will tell you which one.
\end{itemize}

\subsection*{Some example questions (the real ones will be in this spirit)}
...
\end{tabular}
\end{center}
Your Review Lab 1 score is the percentage for the highest level you reliably demonstrate.

\section*{\S7 --- Required submission notes files}
\emph{You must include each of these as a separate file. Any missing file means you cannot sign up for a review.}

\medskip\noindent\textbf{Design note} (\file{design.md}, $\leq$ 1 page): your key-length decision rule (including tie-break and how you handle multiples), your column-scoring statistic, and known failure cases. Not graded for style; used by your interviewer to prepare questions. It's in your interest for it to be accurate.
...
\section*{\S9 --- Submitting your anonymous agent report}

It is perfectly fine to have a coding agent submit your anonymous agent report. In fact, that's exactly how we expect you to use it. The necessary instructions are in {\tt AGENTS.md}, and in the separate handout \file{anon-report.pdf} (in the assignment repository) if you'd like to do it yourself.

For this assignment the channel is a \textbf{pilot}: reports are encrypted to the course public key. They're then signed with your registered key (that you registered in Part 0) and deposited through the anonymous channel using the same \file{tools/anon-report.py} (its \code{deposit} mode). The roster will be published on September 14. Deposits travel over Tor, so please \textbf{install Tor Browser} (free, \url{https://www.torproject.org}) before then.

\medskip\noindent In brief: your report is encrypted on your own machine so that only the instructor can read it, signed with a ring signature that proves it came from an enrolled student without saying which, and deposited over Tor so the server never sees your IP address. What remains is ordinary metadata such as timing and writing style, which we don't inspect. The handout \file{anon-report.pdf} spells out the steps, the dates (deposit by \textbf{Sun Sep 20}; attendance proof by \textbf{Fri Sep 25}), and exactly what the guarantee does and doesn't cover. \textbf{Read your report before it is sent.}


\bigskip
\noindent\rule{\textwidth}{0.4pt}

\medskip\noindent\textbf{Authorship and AI usage note} \emph{(in the same format this course asks of you).} This handout was written by \textbf{Matthew Green} with \textbf{Claude} (Anthropic's Fable~5 model, via Claude Code). Delegated to the AI: recovering and comparing the previous versions of this assignment, drafting the text, the \LaTeX{} conversion, and the autograder and review-lab tooling. Done by the human: the assignment's design --- what is examined, how, and why --- every grading decision, and review and editing of every section. One thing we caught: the first version of the Part~4 experiment grid was too easy --- a correct tool never failed anywhere on it --- which we discovered only by running a reference implementation through it. The grid you have is the one where the boundary is actually visible.
