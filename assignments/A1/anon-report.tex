...
\medskip \noindent
\begin{enumerate}
\item \textbf{Before you start.} Have \textbf{Tor Browser} open and connected (free,
  \url{https://www.torproject.org}; the tool talks to it on port 9150 --- a running
  \code{tor} daemon on 9050 also works). Pull the course repository so that
  \file{anon-report/roster.json} exists --- it contains every registered key, the course
  encryption key, and the drop box's onion address. If the file isn't there yet, the
  channel isn't open.
...
\item \emph{Can I see that my report arrived?} Yes --- step 6. The board is public
...
\medskip\noindent\textbf{Authorship and AI usage note} \emph{(in the same format this course asks of you).} This handout and the initial tool and server were written by \textbf{Matthew Green} with \textbf{Claude} (Anthropic's Fable~5 model, via Claude Code). \textbf{OpenAI Codex} later performed a security review, hardening, adversarial and Tor end-to-end tests, and deployment verification. An early version used a wrong 2048-bit group prime; the current tool embeds the published RFC~3526 value and checks basic group invariants before use. Verify your constants.
